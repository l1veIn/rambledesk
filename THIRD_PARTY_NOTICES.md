...
| X-ASR 480ms streaming zh/en punct int8 | 默认本地流式中英标点模型 | 见上游发布条款 | <https://github.com/k2-fsa/sherpa-onnx/releases/download/asr-models/sherpa-onnx-x-asr-480ms-streaming-zipformer-transducer-zh-en-punct-int8-2026-06-05.tar.bz2> |
...
Sherpa online session 的配置与尾帧策略由 Kotone 的 MIT 实现改写而来：
`kotone/crates/kotone-stt/src/online_transducer.rs`。RambleDesk 改为有界队列、
自然端点 stable 和长会话分段；未复用 Kotone orchestrator。详情见
`docs/KOTONE_REUSE.md`。
