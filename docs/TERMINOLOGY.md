...
> 这是规范目标，不是当前 Desktop 接线快照；ACP 能力与 Adapter Runtime 的隔离进度见 [V3_IMPLEMENTATION_PLAN.md](V3_IMPLEMENTATION_PLAN.md)。

本文是 RambleDesk 的唯一术语源。其他文档只引用本文，不重新定义产品对象。

## 产品定义

RambleDesk 是本地 human-agent workbench。它把 Managed ACP Session 与 Adapter Session 投影到同一个 Workbench，把 Agent 对人类注意力的需求呈现为 Inbox 中的 Permission Request、Ask Question 或 Feedback Request，再把人类的授权、答案或结构化 Feedback Package 按 Session Source 送回原路径。人类也可以用 Launch Ramble 表达并密封任务，主动创建、观察和指挥 Managed ACP Session。
...
2. **ACP 提供主动管理能力，不垄断 Session。** RambleDesk 通过 `rambledesk-acp-client` 实现 ACP Client 角色，为 Managed ACP Session 提供主动 Launch、runtime 状态与配置、Permission、Ask 和恢复；产品和代码不再使用 `rambledesk-acp-host`。
...
4. **v3 Core 区分持久事实与 live 交互。** Managed ACP Session、Ramble Submission、Feedback Request 和 Feedback Package 是核心事实；Agent Run、Prompt Turn、Permission Request 和 Ask Question 是 live 运行交互；Attention Item 只是 read model。不得为了复刻 Agent 会话再复制第二套事实库。
5. **Workbench 汇合投影，不合并 Core。** Managed ACP Session 由 v3 Core 拥有，Adapter Session 由维护冻结的 Adapter Runtime 拥有；Desktop Unified Workbench Projection 可以合并两边的 Session 与 Attention Item，但命令必须按 Session Source 返回原 owner。
...
8. **两个数据代际隔离运行。** v3 use case 只读写新表，Adapter Runtime 只读写其既有 store；禁止跨 Core fallback、同一事实双写或让任一 Core 解释另一方的 schema。
9. **旧数据迁移是可选路径，不是显示前提。** Desktop 可以通过 Adapter Workbench Source 直接展示旧数据并继续原操作；独立迁移器仍可显式、有报告地把可保留事实有损导入 v3，但 App 不自动迁移。
10. **应用生命周期只拥有受管进程生命周期。** 关窗口或进入托盘不结束 Managed ACP Agent Run；退出 RambleDesk 结束所有由它启动的 live Agent Run。Adapter Session 的外部进程与 continuation 仍由原路径拥有。
...
8. Agent 收到确认后结束当前 Prompt Turn；Session 保持 Waiting for Feedback，直到人类提交或明确取消。这个等待可以跨 Prompt Turn、窗口关闭和 Agent Run。重启 RambleDesk 会恢复该 Feedback Request；waiting 项回到待处理 Inbox，terminal 项保留在请求历史，不为了等待人类而提前启动 Agent。
...
### Adapter Session Path（维护冻结）

1. 既有 MCP、Pi 或原生 Adapter 继续通过 Adapter Runtime 的 Core 与 store 创建、读取和处理原有请求；该路径不经过 v3 Core，也不为了显示而复制新表。
2. Desktop 通过 Adapter Workbench Source 把既有 host/session/request 状态投影为 Adapter Session 与对应 Attention Item；投影身份携带 Session Source，不能把既有 id 冒充 v3 `session_id`。
3. 人类打开既有数据时继续使用已有 Ramble Workspace；保存、提交、取消与 continuation 命令按 source 路由回 Adapter Workbench Source，保持原有语义。
4. 旧路径进入维护冻结：允许修复安全、数据完整性和阻断性兼容问题，不承接主动 Launch、ACP Permission、ACP Ask、runtime config 等新能力。

Adapter Session 不因出现在统一 Workbench 就获得 ACP Session Link、Managed Feedback Resume 或 v3 Delivery 语义。两条运行路径的共存边界见 [ADR 005](adr/005-unified-workbench-coexistence.md)。

### Imported Session Path（可选有损迁移）

人类可以显式运行独立迁移器，把旧数据中可解释的事实导入 v3 新表，形成 Imported Session。Imported Session 是迁移快照，不再依赖旧 store，但不因此获得原 Agent 上下文或 Managed ACP continuation。直接使用 Adapter Session 与执行迁移互不构成前置条件；Unified Workbench Projection 必须通过迁移来源映射避免同一旧 Session 重复展示。
...
| 工作台（Workbench） | RambleDesk 桌面产品，包括 Session 导航、Ramble 编辑器、Inbox 和 Context View。 | 拥有人类工作流与跨 source 投影；不直接实现 ACP、MCP 或任一 persistence。 |
| Session Source | 一个 Session 的事实 owner 与命令路由来源。 | 当前为 Managed ACP、Adapter 或 Imported；不是在线状态、Agent 品牌或 UI 分组。 |
| Unified Workbench Projection | Desktop 把不同 Session Source 的 Session、Attention Item 与可用动作合并后的只读 UI 模型。 | 可以统一排序和选择，不是第三个 Core、共享数据库、双写层或跨 source fallback。 |
...
| Session | RambleDesk 中一段可持久、可选择的 human-agent 工作关系。 | 是统一 Workbench 的顶层产品类别；具体事实、身份和可用动作由 Session Source 决定，不要求所有 Session 共享一个 Core 或物理主键。 |
| Managed ACP Session | 由 RambleDesk 经 ACP 主动创建并监督的 Session。 | 由 v3 Core 与 ACP Client 拥有，支持 Launch、Agent Run、ACP Session Link、runtime 状态/配置、Permission、Ask 与 Managed Feedback Resume。 |
| Adapter Session | 由既有 MCP、Pi 或原生 Adapter 路径拥有、经 Adapter Workbench Source 投影到 Workbench 的 Session。 | 由维护冻结的 Adapter Runtime 保持事实与原操作；不复制到 v3、不宣称 ACP 能力。 |
| Imported Session | 由可选有损迁移写入 v3 新表的旧事实快照。 | 可查看可保留内容，但没有原 Agent 上下文或原路径 continuation；不得与来源 Adapter Session 重复展示。 |
| Adapter Runtime | 既有 Core/store、MCP/local-server/host/Pi 等接入路径的隔离运行边界。 | 维护冻结但仍可达；不写 v3 表、不接收 ACP 新能力，也不被 v3 Core 包装成 persistence fallback。 |
| Agent Run | Managed ACP Session 中一次具体的 Agent 启动与连接实例。 | 是 live runtime 对象，拥有进程树和连接状态；默认不建立跨重启历史表，Session 可以比它活得更久。 |
...
| Session Record | v3 Core 为 Managed ACP Session 或 Imported Session 保存的最小持久记录。 | Managed ACP Session 至少包含本地身份、Launch Configuration、ACP Session Link、状态和时间戳；Adapter Session 继续使用其原 store，不创建影子 Record。 |
...
| Attention Item | 请求列表对 Permission Request、Ask Question 和 Feedback Request 的统一 read model。 | waiting 项组成当前待处理 Inbox；Managed ACP terminal Feedback 可以作为 RambleDesk 自有结构化历史继续投影。不要求三类请求共享 id、生命周期、传输或存储表。 |
...
| Feedback Delivery | 把一个已终态 Feedback Request 交回 Managed ACP Session 的持久交付意图。 | 状态为 `pending` 或 `delivered`；用稳定 `delivery_id` 支持 at-least-once 重试和去重。没有可用 Agent Run 时保持 pending，不回滚 Request resolution，也不退化成本地路径或 Adapter Continuation。 |
...
| Managed Feedback Resume | Feedback Request 终态后，在同一 Managed ACP Session 中重新唤起 Agent 获取结果的动作。 | 基线实现是 Feedback Resume Prompt + `get_feedback(request_id)`；直接内联 Envelope 或长等待返回只是可替换优化，不叫 Adapter Continuation。 |
...
| Managed ACP Workbench Source | 把 v3 Core 与 ACP Client 的 Session、Attention Item、runtime capability 和可用命令投影给 Desktop 的隔离 source。 | 是 Session Source 值 `Managed ACP` 的装配边界；不向 UI 泄漏 ACP wire payload。 |
| Adapter Workbench Source | 把 Adapter Runtime 的 Session、Attention Item 与可用命令投影给 Desktop 的隔离 source。 | 与面向 Agent 的 Host Adapter 区分；只做来源映射与命令路由，不把 Adapter Runtime 伪装成 v3 Core，也不重解释既有数据。 |
| Adapter Continuation | Adapter Session 按原 MCP、Pi、host 或 native Adapter 机制把结果交回 Agent 的动作。 | 由 Adapter Runtime 拥有；不复用 Managed Feedback Resume，不要求 v3 `delivery_id`，也不升级为所有 Session 的统一 continuation。 |
...
### Session、Session Source 与 ACP Session

Managed ACP Session 的 `session_id` 是 v3 Core 持久身份，Adapter Session 保留原 owner 的 native id；Desktop 选择态使用同时携带 source 与 native id 的 `workbench_session_ref`。`acp_session_id` 是某次 ACP 关联中的不透明协议 id。三者不得相等假设、互相回退或作为彼此的兼容别名。
...
Feedback Request 的等待没有业务超时，且可以比发起它的 Prompt Turn 和 Agent Run 活得更久。Managed ACP Session 中的 `request_feedback` 在持久化成功后快速返回 ack；这个 ack 不解决请求，只让 Agent 安全结束当前 Prompt Turn。因此无限等待由 RambleDesk 的持久状态保证，不依赖 MCP `tools/call` 一直占用连接。
...
Attention Item 是请求列表的 read model，不是协议对象。列表可以同时显示 Permission、Ask Question 和 Feedback，也可以合并 Managed ACP 与 Adapter 两个 source，但每一项必须保留自身 source、操作、终态和身份字段。Managed ACP Session 重启后会从 v3 重建全部 Feedback Request；只有 `waiting` 项属于待处理 Inbox，`submitted` / `cancelled` 项是只读结构化历史。Permission Request 和 Ask Question 随原 Run 取消，不伪装成可回答的旧请求。Adapter Session 的可恢复性服从原 owner。Feedback 已提交但 Delivery 仍 pending 时，Delivery 另显示为 Session 的交付状态，terminal Request 本身也不能伪装成未回复项。
...
| 字段 | 目标语义 | 规则 |
| --- | --- | --- |
| `workbench_session_ref` | Desktop Unified Workbench Projection 中的 Session 选择与命令路由身份。 | 由 Session Source + source-native id 组成；不得只用裸 id 合并不同 Core。 |
| `session_id` | v3 Core 的唯一持久 Session id。 | 用于 Managed ACP Session 与 Imported Session；不得以进程 id、ACP id 或旧 host session id 代替。 |
| `session_kind` | v3 Session Record 的创建与监督方式。 | 当前为 `managed` 或 `imported`；不是 Adapter Session 的 source 标记或 transport 状态。 |
| `agent_profile_id` | 稳定 Agent 家族 id。 | 用于用户识别和能力归属；不选择具体 executable。 |
| `launch_profile_id` | 具体启动配置 id。 | Managed ACP Session 必须有；Imported Session 可以为空；Adapter Session 不借此宣称可由 ACP 启动。 |
...
旧字段 `host_id`、`host_session_id` 可以留在 Adapter Runtime 的既有数据与迁移器输入模型中；不得进入 v3 Core DTO/新表成为 alias 或 fallback。Unified Workbench Projection 必须在 Adapter Workbench Source 内将它们封装为 source-native identity，不能泄漏为通用 Session 字段。
...
### Adapter Session Path

既有 MCP、local server、host、Pi 与原生 Adapter 继续通过 Adapter Runtime 支持原有 Feedback 操作：

- 旧 Core/store 是本路径自己的事实 owner，不调用 v3 Core，也不向新表双写。
- Desktop 只能经 Adapter Workbench Source 读取投影和执行命令；不得让 UI controller 直接跨越两个 owner。
- 旧 listener、token、JSON API、MCP tool 与 continuation 留在隔离边界内；它们不获得 ACP 生命周期、Permission、Ask 或 runtime config 能力。
- 反馈结果按原路径交付；Managed Feedback Resume 与 `get_feedback` Delivery 合同只属于 Managed ACP Session。
- 维护冻结允许安全、数据完整性与阻断性兼容修复，不承诺功能齐平，也不预设删除日期。

### Unified Workbench Projection

Desktop 分别从 Managed ACP Workbench Source 与 Adapter Workbench Source 获取 source-tagged snapshot，再合并为 Session list 与 Inbox。相同标题、Agent 或 workspace 不是去重依据；只有显式迁移来源映射可以把 Imported Session 与其 Adapter Session 视为同一来源事实。选择、保存、提交、取消、Permission 回答和 Ask 回答都按 `workbench_session_ref` 路由，禁止“先试 v3、失败再试 Adapter”。

## Package 边界

| Package / 区域 | 职责 | 不应包含 |
| --- | --- | --- |
| `crates/rambledesk-core` | Managed ACP Session 与 Imported Session 的 v3 Core：Session Record、Launch Configuration、ACP Session Link、Ramble Intent/Submission、Feedback Request、Feedback Package、Delivery 的领域 DTO、use case 与 ports，以及 Agent Run、Permission、Ask Question 的协议中立 runtime DTO。 | ACP/MCP/HTTP/JSON、进程管理、Agent transcript、Tauri command、厂商启动命令、绝对存储路径、Adapter Runtime schema 或 fallback。 |
| `crates/rambledesk-storage` | v3 Core 的 SQLite Adapter、结构化 Draft、Session Record 及 Launch Configuration、Ramble Submission、Feedback Request/Package/Delivery，以及本地 content-addressed Artifact Store Adapter。 | ACP/MCP 协议、进程监督、Agent transcript mirror、Permission/Ask Question 历史表、Adapter store 运行时读取、把本地路径泄漏为领域合同。 |
...
| `crates/rambledesk-local-server` | Adapter Runtime 的既有 authenticated local transport 与 routes。 | v3 Core 规则、ACP 会话实现、新能力入口、向 v3 表双写。 |
| `crates/rambledesk-mcp` | Adapter Runtime 的既有 MCP tool 与安装能力。 | v3 Session 真源、ACP 生命周期、新 Agent catalog、Managed Feedback Resume。 |
| `crates/rambledesk-hosts` | Adapter Runtime 的既有外部工具探测与 continuation strategy。 | v3 Session 模型、ACP Client 或 Launch Configuration owner。 |
| `packages/pi-rambledesk` | Adapter Runtime 的 Pi 原生流程。 | Managed ACP Session 语义、Desktop projection owner、v3 storage 写入。 |
| `packages/dsh-rambledesk` | Adapter Runtime 的 DSH 接入与 installer。 | Managed ACP Session 语义或 ACP Runtime 依赖。 |
| `apps/desktop` | composition root、Unified Workbench Projection、source-aware command routing、Workbench UI、Tauri wiring、应用与托盘生命周期。 | 领域持久化真源、ACP JSON-RPC、Adapter query 细节、跨 Core fallback 或双写。 |
...
- Adapter Runtime 保持自己的 Core/store；v3 Core 与 Adapter Runtime 不互相依赖、不共享 repository port。
- Session Tool Companion 是 `rambledesk-acp-client` 使用的 MCP adapter，不绕过 `core` use case 直接操作数据库。
- `apps/desktop` 分别组装 Managed ACP Workbench Source 与 Adapter Workbench Source，只在 Unified Workbench Projection 汇合 source-tagged read model 与命令路由。
- ACP Agent SDK 只能出现在 `rambledesk-acp-client` 一侧，不能沿 DTO 泄漏到 Svelte 或 `core`。

## 数据代际与迁移规则

本次重构建立 v3 数据代际，同时保留隔离的 v2 数据代际：
...
- Adapter Runtime 继续只读写既有 store；它是独立 source，不是 v3 repository 的 fallback read。v3 use case 不探测旧字段来改变行为。
- 不做新旧双写，不在任一 Core 内做跨代 `COALESCE`、alias DTO 或反序列化 fallback；Desktop 的 Unified Workbench Projection 是两个 Adapter 输出的合并，不是 database view。
...
- 迁移结果写入新表后只按 v3 合同解释；其来源映射用于避免与仍可达的 Adapter Session 重复展示。旧 store 不因迁移自动删除或停用。
- 迁移器是可选交付工具，不是 Desktop 正常读取旧数据的前置步骤，也不是永久 Adapter。
...
- Session Source / Unified Workbench Projection
- Adapter Workbench Source / Adapter Runtime
- MCP
...
- 把 Managed ACP Session 的 Feedback Delivery 称为 Adapter Continuation，或把两条 continuation 强行归一为同一协议。
...
- Desktop Unified Workbench Projection 同时展示 Managed ACP Session、Adapter Session 和可选 Imported Session；所有选择与命令携带 Session Source，不按裸 id、标题或失败回退猜 owner。
...
- `host_id` / `host_session_id` 只留在 Adapter Runtime 的既有数据和迁移输入；v3 Core 不读取、不别名化。两个 owner 不双写、不互相 fallback。
- ACP 新增主动 Launch、runtime 状态/配置、Permission、Ask 与 Managed Feedback Resume；MCP/Pi/原生 Adapter 继续服务 Adapter Session 的原操作，不伪装能力齐平。
