...
执行已有结果但保存失败时，只重试保存结果，不重发 prompt；重启将遗留 `sending` 标为 `uncertain`。该条不会自动重发，但之后新提交的反馈仍会按条投递。删除先保存意图、封锁输入与反馈、撤销凭据，再回收本实例及所属记录；失败保留状态供重试。项目目录、Agent 自己的历史和用户独立启动的服务不在删除范围内。
