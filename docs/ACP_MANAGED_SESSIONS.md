...
| `delivered` | 续接消息已发给 Agent，或用户确认已处理；不等于这一轮或整个任务完成。 |
| `uncertain` | 无法确定该条已处理多少内容，只能由用户对该条明确重试或确认。不阻止同一会话里后续已提交反馈的投递。 |
| `discarded` | 删除等操作撤销投递，不再续接。 |

执行已有结果但保存失败时，只重试保存结果，不重发 prompt。投递成功以续接消息已写入会话并开始发送为准，不要求这一轮 EndTurn。重启将尚未完成发送的 `sending` 标为 `uncertain`；该条不会自动重发，但之后新提交的反馈仍会按条投递。删除先保存意图、封锁输入与反馈、撤销凭据，再回收本实例及所属记录；失败保留状态供重试。项目目录、Agent 自己的历史和用户独立启动的服务不在删除范围内。
