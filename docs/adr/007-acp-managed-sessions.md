...
生产托管入口统一为应用可执行文件的 `feedback request/get/recover` 命令。Agent 经实例私有本地 IPC 通道调用，
控制器保管令牌并转发至 `/agent-feedback/*` HTTP JSON。Unix socket 位于权限 0700 的临时目录，Windows 使用拒绝远程
客户端的随机命名管道。停止/替换实例时关闭通道；旧通道不能回退到 HTTP 或外部 API。
共享命令模块不启动 UI、另建后端或打开数据库。实例私有环境只提供可执行绝对路径、本地通道地址与托管标记，
不再向 Agent 传 HTTP URL/bearer，避免 DSH 等子进程清理 KEY/TOKEN/SECRET 环境变量时破坏反馈。
生产 ACP 不注入 MCP server、stdio companion 或 Pi 托管扩展。
...
公共驱动按真实命令回执检查每轮交接；回答、总结、只读任务同样需要 Ramble。Agent 正常结束却未调用反馈时，
只追加一次不重复工作的交接提醒；已尝试失败或提醒后仍无回执则保留回答并显示错误，不循环催促或代替模型创建请求。
用户取消当前轮次不触发提醒。明确跳过/完成/取消可用 `feedback skip --reason ...` 记录仅本轮有效的例外；读取已批准或
已取消的请求也可确认终态。续接后读取普通已提交反馈不等于下一轮交接。迟到的上一轮响应不能满足新轮次的检查。
