...
模型、模式等选项按后端实际提供的能力展示。只有完成本项目反馈闭环验收的入口才标记为已验证；目录中的其余入口保留明确的证据边界。
...
| F2 Pi 专用通道 | 为忽略 MCP server 配置的 Pi bridge 提供自有 wrapper 与扩展 | 原生扩展、私有请求、load/resume、启动中停止、删除隔离与子进程令牌清理 |
