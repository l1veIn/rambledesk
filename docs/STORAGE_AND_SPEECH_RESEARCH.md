...
1. 设置名称使用“资料库位置”还是“数据存储位置”？建议前者，避免用户误以为 token/数据库也会移动。
2. 首版更改位置后，是否直接提示清空旧开发数据？建议是。
3. 发布后的 Feedback Package 是否永久保留，还是未来提供按时间/大小清理策略？
4. 草稿附件在 package 成功发布后是否可以自动回收？若可以，可显著避免附件双份占用。
5. 资料库不可用时，应用应完全阻止新反馈，还是允许只读浏览数据库元数据？建议只读浏览 + 禁止写入。
