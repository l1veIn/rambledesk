...
- 服务只绑定 IPv4 loopback；
- 业务工具将在 M1 使用 durable request + `get_feedback` polling；
- Tasks 只在客户端显式声明支持后作为增强路径。

## 2. 客户端实测

| 客户端 | Transport | 协商协议 | 自定义 header | Tasks 声明 | health 调用 | 结论 |
|--------|-----------|----------|---------------|------------|-------------|------|
| MCP Inspector 2.0.0 CLI | Streamable HTTP | `2025-11-25` | `--header` 通过 | `true` | 通过 | 作为协议 smoke 与 CI 验证器 |
| Claude Code 2.1.207 | HTTP MCP | `2025-11-25` | 配置 `headers.Authorization` 通过 | `false` | 通过，约 4.4 秒完成 | 首发必须支持 polling |
| Codex CLI（本机安装） | 未能启动 | 未验证 | 未验证 | 未验证 | 未验证 | 环境阻塞，不判定产品不兼容 |

Claude Code 使用 `--strict-mcp-config` 和只允许
`mcp__rambledesk__rambledesk_health` 的非交互调用完成验证。服务端观察到
`clientSupportsTasks: false`，因此不把 Tasks 作为正确性前提。
