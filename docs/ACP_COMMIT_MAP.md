...
- `fix(desktop): forward application changes to native session views`：Desktop composition root 补装 ApplicationChangeHub → Tauri 事件流桥接，实例退出时释放；接收落后时发 ready 触发快照重读。前端首次快照等待原生监听注册完成，注册失败保持可见。验收：2 项原生 Tauri emit/listen 集成测试覆盖连续 36 次会话变更与 512 条积压后的恢复；前端覆盖 36 次持续刷新、最终 idle、双会话隔离、旧 prompt 返回和异步监听注册空窗，全量 104 文件/581 项 TS 及 Svelte 检查通过。Windows/MSVC 的 integration tests 复用 Tauri 已生成的 manifest 资源，不改变生产 binary 链接。此前 Web 手工验收未覆盖这条原生事件接线。
