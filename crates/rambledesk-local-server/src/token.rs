...
#[derive(Debug, Error)]
pub enum TokenError {
    #[error("local server access token must be exactly 64 hexadecimal characters")]
    InvalidFormat,
    #[error("failed to access local server token file: {0}")]
    Io(#[from] io::Error),
...
pub fn default_token_path() -> Result<PathBuf, TokenError> {
    dirs::data_local_dir()
        .map(|root| {
            root.join("RambleDesk")
                .join("auth")
                .join("local-server.token")
        })
...
mod tests {
    ...
    fn invalid_token_files_are_rejected() {
        let directory = tempfile::tempdir().expect("temp dir");
        let path = directory.path().join("local-server.token");
