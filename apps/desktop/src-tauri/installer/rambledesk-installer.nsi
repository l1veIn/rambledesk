...
; RambleDesk's zero-dependency MUI2 theme. All resources are checked into the
; repository, so local and CI builds use the same inputs. Light ice matches the
; app; native checkboxes stay readable on this background.
!define MUI_BGCOLOR "F7F9FC"
!define MUI_TEXTCOLOR "20334B"
!define MUI_INSTFILESPAGE_COLORS "20334B F7F9FC"
...
; 1. Welcome Page
!define MUI_WELCOMEPAGE_TITLE "$(rambleWelcomeTitle)"
!define MUI_WELCOMEPAGE_TEXT "$(rambleWelcomeText)"
...
!define MUI_FINISHPAGE_TITLE "$(rambleFinishTitle)"
!define MUI_FINISHPAGE_TEXT "$(rambleFinishText)"
...
LangString rambleFinishText ${LANG_ENGLISH} "RambleDesk is installed. Turn on ramble mode, then wait for the knock."
LangString rambleAppRunningBlock ${LANG_ENGLISH} "RambleDesk is currently running (possibly an elevated instance).$\r$\n$\r$\nPlease quit RambleDesk, then run this installer again."
...
Section Install
  ...
  ${If} $R7 > 0
    MessageBox MB_OK|MB_ICONEXCLAMATION "$(rambleAppRunningBlock)"
    Abort
  ${EndIf}
