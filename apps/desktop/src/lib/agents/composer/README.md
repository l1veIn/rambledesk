...
`AgentComposer.svelte` accepts controlled `value`, `draftKey` (the local session ID), `onchange(text)`, and `onsubmit(text)`. The parent owns draft persistence and clearing after a successful submission. Only clear the submitted draft if the current draft still equals that submission; the user can write the next message while the agent works. Prefer a keyed component per session as well as passing `draftKey`.
...
Shared modules keep the live editor and headless tests on the same Tiptap schema. `to-prompt-blocks.ts` and `from-prompt-blocks.ts` support text blocks only in this slice. Unsupported block kinds fail explicitly rather than being silently dropped. No Markdown parser or formatting input rules are loaded.
