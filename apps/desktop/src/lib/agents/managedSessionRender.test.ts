...
describe('Managed session rendering', () => {
  it('keeps unsupported questions visible and cancellable with answer submission disabled', () => {
    ...
    expect(buttons.find(button => button[2].includes('Submit answer'))?.[1]).toMatch(/\sdisabled(?:=|\s|$)/)
    expect(buttons.find(button => button[2].includes('Decline question'))?.[1]).not.toMatch(/\sdisabled(?:=|\s|$)/)
    expect(buttons.find(button => button[2].includes('Cancel question'))?.[1]).not.toMatch(/\sdisabled(?:=|\s|$)/)
  })
  it('renders agent questions as a form with a separate submit action', () => {
    const action = vi.fn()
    ...
    expect(body).toContain('Which approach?')
    expect(body).toContain('Small change')
    expect(body).toContain('Additional context')
    expect(body).toContain('Submit answer')
    expect(body).not.toContain('Cancel permission')
    expect(body).not.toContain('Operation details')
    expect(action).not.toHaveBeenCalled()
  })
  it('renders plan review separately from questions and operation permissions', () => {
    const action = vi.fn()
    const snapshot = pageSnapshot()
    snapshot.runtime.activity = 'waiting_input'
    const { body } = render(ManagedSessionWorkspace, { props: {
    ...
    expect(body).toContain('First update the contract, then test the callers.')
    expect(body).toContain('Submit decision')
    expect(body).toContain('Decline plan')
    expect(body).toContain('Cancel review')
    expect(body).not.toContain('Submit answer')
    expect(body).not.toContain('Cancel permission')
    expect(action).not.toHaveBeenCalled()
  })
  function pageSnapshot(): ManagedSessionViewSnapshot {
    return {
      ...
      runtime: { configuration: { options: [{ id: 'model', name: 'Model', description: null, category: 'model', kind: { type: 'select', current_value: 'fast', options: [{ value: 'fast', name: 'Fast model', description: null, group: null }] } }] },
  ...
  it('keeps the heading focused and moves project details below the shared-width composer', () => {
    ...
    const { body } = render(ManagedSessionWorkspace, { props: {
      ...
      onCancel: action, onStart: action, onRespondInteraction: action, onSetConfiguration: action,
    } })
    ...
    expect(body).toContain('Message the agent')
  ...
  it('only offers a connection retry for a visible error and never during deletion', () => {
    const action = vi.fn()
    const snapshot = pageSnapshot()
    const props = { snapshot, onPrompt: action, onCancel: action, onStart: action, onRespondInteraction: action }
    const ready = render(ManagedSessionWorkspace, { props }).body
    expect(ready).not.toContain('Retry')
    expect(ready).not.toContain('data-agent-setup-guide')
  ...
  it('offers read retry for query failures without treating them or turn failures as connection attempts', () => {
    ...
    const base = pageSnapshot()
    const props = { onPrompt, onStart, onRefresh, onCancel: vi.fn(), onRespondInteraction: vi.fn() }
  ...
  it('initially mounts twenty prompts and keeps an accessible fallback for loading older history', () => {
    const snapshot: ManagedSessionViewSnapshot = {
      ...
      runtime: { configuration: { options: [] }, connection: 'connected', activity: 'idle', instance_id: 'instance', config_updated_at: null,
    ...
    const action = vi.fn()
    const { body } = render(ManagedSessionWorkspace, { props: {
      snapshot, onPrompt: action, onStart: action, onCancel: action, onRespondInteraction: action, onLoadOlder: action,
  ...
  it('shows only the active permission details as escaped, redacted text before approval', () => {
    const snapshot: ManagedSessionViewSnapshot = {
      ...
      session: { session_id: 'local-details', host_id: 'dsh', host_session_id: 'feedback-details', title: 'Permission context',
        ...
        management: { kind: 'managed', protocol: 'acp', agent_config_id: 'config', cwd: '/repo', remote_session_id: 'remote' } },
      runtime: { configuration: { options: [] }, connection: 'connected', activity: 'waiting_input', instance_id: 'instance', config_updated_at: null,
  ...
  it('keeps history and the deletion warning while disabling work for a deleting session', () => {
    const snapshot: ManagedSessionViewSnapshot = {
      ...
      session: { session_id: 'deleting-render', host_id: 'dsh', host_session_id: 'feedback-deleting', title: 'Cleanup incomplete',
        created_at: 'today', updated_at: 'today',
        management: { kind: 'managed', protocol: 'acp', agent_config_id: 'config', cwd: '/repo', remote_session_id: 'remote' } },
      runtime: { configuration: { options: [] }, connection: 'connected', activity: 'running', instance_id: 'instance', config_updated_at: null,
    ...
    expect(body).toMatch(/<button[^>]*disabled[^>]*>Message the agent/)
  ...
  it('keeps a new session free of manual connection and deletion controls', () => {
    const snapshot: ManagedSessionViewSnapshot = {
      ...
      runtime: { configuration: { options: [] }, connection: 'stopped', activity: 'idle', instance_id: null,
    ...
    const { body } = render(ManagedSessionWorkspace, { props: {
      ...
      onRespondInteraction: runtimeAction,
    } })
  ...
  it('shows only current-session activity and the first permission without starting or stopping anything', () => {
    const snapshot: ManagedSessionViewSnapshot = {
      ...
      runtime: { configuration: { options: [] }, connection: 'connected', activity: 'waiting_input', instance_id: 'instance-one',
