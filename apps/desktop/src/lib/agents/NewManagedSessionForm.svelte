...
<form class="w-full max-w-xl space-y-5 rounded-xl border bg-background p-5" onsubmit={(event) => { event.preventDefault(); void create() }}>
  ...
  {:else}
    <fieldset disabled={locked} class="m-0 min-w-0 space-y-4 border-0 p-0 disabled:opacity-60">
      ...
      <label class="block space-y-1.5 text-xs"><span class="font-medium">{tr('Session title (optional)')}</span><input bind:value={title} class="h-9 w-full rounded-md border bg-background px-3 outline-none focus:ring-2 focus:ring-ring" autocomplete="off" /></label>
    </fieldset>
