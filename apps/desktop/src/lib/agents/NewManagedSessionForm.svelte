<script lang="ts">
  import { FolderOpen, LoaderCircle, Plus } from '@lucide/svelte'
  ...
  async function chooseDirectory() {
    if (locked || !onChooseDirectory) return
    try {
      const selected = await onChooseDirectory()
      if (selected) cwd = selected
...
<form class="w-full max-w-xl space-y-5 rounded-xl border bg-background p-5" onsubmit={(event) => { event.preventDefault(); void create() }}>
  ...
  {:else}
    <fieldset disabled={locked} class="m-0 min-w-0 space-y-4 border-0 p-0 disabled:opacity-60">
      <label class="block space-y-1.5 text-xs"><span class="font-medium">{tr('Agent')}</span><select required bind:value={configId} class="h-9 w-full rounded-md border bg-background px-2"><option value="" disabled>{tr('Choose an agent')}</option>{#each enabledConfigs as config (config.id)}<option value={config.id}>{config.name}</option>{/each}</select></label>
      <div class="space-y-1.5 text-xs"><label for="managed-project-directory" class="font-medium">{tr('Project directory')}</label><div class="flex gap-2"><input id="managed-project-directory" required bind:value={cwd} class="h-9 min-w-0 flex-1 rounded-md border bg-background px-3 font-mono outline-none focus:ring-2 focus:ring-ring" autocomplete="off" spellcheck="false" />{#if onChooseDirectory}<Button type="button" variant="outline" size="sm" onclick={() => void chooseDirectory()}><FolderOpen class="size-3.5" />{tr('Browse…')}</Button>{/if}</div><span class="block text-[11px] leading-5 text-muted-foreground">{tr('Use an absolute directory on the computer running RambleDesk.')}</span></div>
