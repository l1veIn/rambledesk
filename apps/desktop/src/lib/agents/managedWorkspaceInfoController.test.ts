...
describe('visible Agent workspace metadata', () => {
  ...
  it('lets the next scheduled refresh recover after a read deadline', async () => {
    vi.useFakeTimers()
    const pending = deferred()
    const transport = new TestApplicationTransport(undefined, { initiallyReady: true })
      .handle('getManagedWorkspaceInfo', () => pending.promise)
    ...
    controller.start()
    await vi.advanceTimersByTimeAsync(30_000)
    transport.resolve('getManagedWorkspaceInfo', { cwd: '/current', branch: 'recovered' })
    await vi.advanceTimersByTimeAsync(15_000)
    expect(transport.calls).toHaveLength(2)
    expect(get(controller)?.branch).toBe('recovered')
    pending.resolve({ cwd: '/stale', branch: 'stale' })
    await flush()
    expect(get(controller)?.branch).toBe('recovered')
    controller.dispose()
  })
