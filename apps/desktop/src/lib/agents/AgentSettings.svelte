<script lang="ts">
  import { CheckCircle2, Eye, EyeOff, LoaderCircle, Save, Trash2 } from '@lucide/svelte'
  ...
  export let cache = new AgentDraftCache()
  export let baselines = new Map<string, string>()
  let draft = cache.select(configs[0]?.id ?? null, configs)
  let initialized = true
  if (configs[0] && !baselines.has(configs[0].id)) baselines.set(configs[0].id, JSON.stringify(agentConfigDraft(configs[0])))
  ...
  $: if (configs[0] && JSON.stringify(configs[0]) !== sourceSignature && pending === null) {
    draft = cache.reconcile(configs[0])
    baselines.set(configs[0].id, JSON.stringify(agentConfigDraft(configs[0])))
    sourceSignature = JSON.stringify(configs[0])
  }
...
<section class="min-w-0" aria-label={tr('Agent details')}>
    <form class="min-w-0 space-y-4 rounded-lg border bg-background p-4" onsubmit={(event) => { event.preventDefault(); void save() }} onfocusin={() => initialized = true}>
      ...
      <p class="m-0 text-[11px] leading-5 text-muted-foreground">{tr('Checks verify the ACP channel and required feedback capability. Model options and authentication are handled when you prepare and use a real session.')}</p>
    </form>
</section>
