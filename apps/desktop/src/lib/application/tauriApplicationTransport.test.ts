...
function managedSnapshot(sessionId: string, sequence: number): ManagedSessionSnapshot {
  return {
    ...
    runtime: { configuration: { options: [] }, connection: 'connected', activity: sequence === 36 ? 'idle' : 'running', instance_id: `instance-${sessionId}`, config_updated_at: null,
    ...
    interactions: [], deliveries: [], deleting: false, recovery: null,
  }
}
