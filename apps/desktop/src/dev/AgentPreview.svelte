...
<div class="flex h-screen flex-col bg-background text-foreground">
  ...
  {:else if page === 'draft'}<main class="mx-auto flex min-h-0 min-w-0 w-full flex-1 flex-col">{#key draftId}<DraftManagedSessionWorkspace {transport} controller={draft} {draftId} onConfigure={() => page = 'agents'} onChooseDirectory={async () => previewProjectDirectory} />{/key}</main>
  ...
  {:else}<main class="mx-auto flex min-h-0 min-w-0 w-full max-w-5xl flex-1 flex-col border-x">{#key sessionId}<ManagedSessionSection {transport} {sessionId} />{/key}</main>{/if}
  </div>
